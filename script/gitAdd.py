...
workingDirectoryFiles = set()  # for now
...
def addDir(path):
    for p in pathlib.Path(path).iterdir():
        if p.is_file() and p not in trackingArea:
            ...
            trackedFiles.add(p)
            workingDirectoryFiles.remove(p)
        elif p.is_dir():
            addDir(p)
...
def gitAdd(p):
    ...
    for element in p:
        ...
        if absolutePath.is_file():
            ...
            workingDirectoryFiles.remove(absolutePath)
...
def gitStatus():
    ...
    untrackedFiles = workingDirectoryFiles - trackedFiles #tracking area
